...
- The e2e suite now has a full-path proof for native top-document actions, coordinate click/hover, same-origin iframe click, stale frame-ref recovery, ancestor/frame-selector native DOM locator targeting, offscreen scroll success, detached-target retry, common actionability rejections, closed-shadow rejection, unsupported cross-origin/sandboxed frame diagnostics, timeout diagnostics, and current wait primitives. The remaining Playwright/Cypress parity gaps are deeper overlay/actionability edges, broader frame ambiguity/navigation-reload coverage, and raw CDP failure coverage.
