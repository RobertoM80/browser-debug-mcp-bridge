...
The project has a real-browser live automation feature, but the current implementation is not equivalent to Playwright or Cypress behavior. It reuses the bound Chrome extension session, which is the right product direction, and the primary top-document, open shadow-root, and same-origin iframe actions now use a CDP-backed native execution backend. Frame refs now carry URL/title metadata for stale-id recovery, frame-aware actions recover across iframe reload/replacement by re-evaluating frame URL/title/selector context, unsupported cross-origin/sandboxed pointer frames return explicit policy diagnostics, `target.locator` now resolves through the extension's native DOM path for direct/workflow actions with ancestor and same-origin frame-selector composition, and the MCP server now has production-flow preflight plus URL/navigation/navigation-lifecycle/load-state/selector/console/dialog/stable-layout/download/popup/network-quiet/request/response waits. The remaining gaps are narrower locator parity, broader frame edge coverage, and the last actionability/diagnostics refinements.
...
The deep audit added a full-path Playwright spec at `apps/e2e-playwright/tests/full.live-automation.spec.ts`. It proves MCP -> extension -> bound tab execution for top-document click, hover, input, key press, focus, blur, scroll, submit, open shadow-root click, same-origin iframe click, nested same-origin iframe click, stale frame-ref recovery, ancestor/frame-selector native locator targeting, `run_ui_steps`, `get_interactive_elements`, native actionability rejections, first-class wait primitives, stale frame diagnostics, and automation-history trace lookup. The implementation now routes `click`, `hover`, `input`, `press_key`, `focus`, `blur`, `scroll`, and `submit` through a CDP-backed native backend (`cdp-native-v2`) in `apps/chrome-extension/src/automation-native.ts`. Same-origin frame and open shadow-root discovery is implemented in `get_page_state` and `get_interactive_elements`. Full DOM locator parity and complete actionability parity still remain open, but the missing wait primitives are no longer the main blocker.
...
- Same-origin iframe click and nested same-origin iframe click are now covered end to end. Cross-origin, sandboxed opaque-origin, or inaccessible iframe pointer actions remain diagnostic because the native click driver must translate frame-local coordinates into top-document CDP coordinates. Native results include `framePolicy`, `frameCoordinateResolved`, and `frameResolution` diagnostics for these cases.
- Targeting supports CSS selectors, frame-aware element refs, open shadow-root selectors, top-document coordinate click/hover targets, compact semantic matching, and native DOM locator resolution for same-element role/name/exact/regex/positional helpers plus explicit ancestor/descendant relations and same-origin frame-selector composition. Closed shadow-root traversal now fails explicitly with `closed_shadow_root_unsupported`. Remaining gaps are arbitrary selector state and full locator semantics comparable to modern browser test tools.
- Waits understand compact page-state summaries, URL exact/contains/regex predicates, persisted navigation events, live navigation lifecycle states, live document readiness for `domcontentloaded`/`load`, selector attached/detached/visible/hidden state, live console messages, native JavaScript dialogs, stable layout windows, downloads, popups, bounded persisted network quiet windows, and request/response predicates. Timed-out waits now return matcher summaries and last-observed diagnostics; the remaining gap is broader fixture coverage and deeper raw-event detail, not missing wait primitives.
- The e2e suite now has a full-path proof for native top-document actions, coordinate click/hover, same-origin iframe click, stale frame-ref recovery, iframe reload/replacement recovery, ancestor/frame-selector native DOM locator targeting, explicit frame ambiguity diagnostics, offscreen scroll success, detached-target retry, common actionability rejections, closed-shadow rejection, unsupported cross-origin/sandboxed frame diagnostics, timeout diagnostics, and current wait primitives. The remaining Playwright/Cypress parity gaps are deeper overlay/actionability edges, broader frame edge coverage, and raw CDP failure coverage.
...
Deep audit update: the full-path action proof now asserts `cdp-native-v2` for click, hover, input, key press, focus, blur, scroll, submit, open shadow-root click, same-origin iframe click, nested same-origin iframe click, stale frame-ref recovery, ancestor/frame-selector native DOM locator targeting, offscreen scroll success, detached-target retry, workflow, current first-class waits, and automation-history trace lookup behavior. The remaining coverage gap is deeper actionability edge cases, broader cross-origin/sandboxed frame fixtures, and the remaining locator parity surface.
...
- broader locator ambiguity diagnostics beyond the current frame-resolution samples
...
- Item 6 has the wait-engine slice for URL, navigation-event, navigation-lifecycle, document load-state, selector-state, console, dialog, stable-layout, download, popup, network-quiet, request, and response waits, including workflow integration. Remaining scope is narrower locator parity, broader frame edge coverage, wider navigation-grade frame stability, deeper wait diagnostics, and broader raw CDP failure coverage.

## Additional Tests To Add

The new full-path spec should be expanded after the native-input refactor starts. Recommended additions:

1. Click actionability: deeper overlay/pointer-capture cases and broader cross-origin frame surface coverage.
...
4. Frames: broader cross-origin/sandboxed fixture coverage and frame ref stability across more reload/navigation patterns beyond the current URL/title/selector recovery baseline.
5. Locators: arbitrary selector state and richer ambiguity diagnostics beyond the current ancestor/descendant/frame-selector native locator baseline.
6. Waits: broader cross-origin/sandboxed fixture coverage for the current wait set and deeper raw-event diagnostics.
7. Diagnostics/history: broader end-to-end assertions for linked failure evidence snapshots and deeper raw CDP failure metadata after real MCP-triggered actions.
...
This feature does not need to open a new Chromium instance. The existing architecture supports real-browser top-document, open shadow-root, and same-origin iframe automation now, including frame-aware refs from page-state capture, stale frame-ref recovery across iframe reload/replacement, ancestor/frame-selector native locator targeting, explicit unsupported-frame diagnostics, production-flow preflight, and first-class URL/navigation-event/navigation-lifecycle/load-state/selector/console/dialog/stable-layout/download/popup/network-quiet/request/response waits. The big remaining pieces are narrower locator parity, deeper diagnostics coverage, broader frame edge coverage, wider navigation-grade frame stability, and the last actionability edge cases.
