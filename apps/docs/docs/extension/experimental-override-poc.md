...
The MCP server also now exposes an early SSR helper path for projects that fetch data on the server before the browser ever sees the response. `discover_ssr_mockability` scans a local project for env-driven base URLs and central request clients, `apply_ssr_mock_config` comments the old env value and writes a managed temporary mock base URL, and `remove_ssr_mock_config` restores or removes that managed patch. The mock base URL can point at the built-in `/mock/ssr/<scope>` Fastify endpoint, which resolves enabled `ssr` and `both` mock routes by request path and records mock run/hit audit rows. This does not magically intercept arbitrary hardcoded SSR calls; it only helps when the target app already has a mockable env or central client path.

The bridge also now persists a first-class mock route model. `create_mock_route`, `update_mock_route`, `delete_mock_route`, `list_mock_routes`, `get_mock_route`, `get_mock_run_log`, `get_mock_hit_log`, and `get_mock_status` let MCP store browser/SSR mock definitions with inline JSON/text/base64/file payload metadata. New persisted routes are inert unless `enabled: true` is set. Enabled `browser` and `both` routes are exposed to the extension as virtual API response override rules, fulfilled through the existing Chrome debugger path, and mirrored into mock run/hit audit rows.
...
The MCP server also has an agent loop guard for override workflows. It stores recent attempts in `mcp_tool_invocations`, opens `mcp_loop_incidents` for repeated unchanged failures, returns `loopGuard.status: "warning"` before blocking, and then blocks repeated side-effecting calls such as `enable_overrides`, `observe_override_assets`, `capture_override_response_body`, `plan_override_response_patch`, `plan_next_source_override`, and `map_next_override_assets`. A blocked response includes `nextActions[0].code = "CHANGE_STATE_BEFORE_RETRY"` plus the required state change, such as reconnecting the session, loading the target route, observing assets, or editing the config. For controlled diagnostics only, set `MCP_LOOP_GUARD=0`.
