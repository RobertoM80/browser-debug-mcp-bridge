...
- Waits now cover compact page-state polling, URL predicates, persisted navigation events, live document readiness for `domcontentloaded`/`load`, arbitrary selector attached/detached/visible/hidden state, live console messages, native JavaScript dialogs, persisted network quiet windows, and request/response predicates as workflow primitives. They still do not cover full navigation lifecycle events beyond document readiness, layout stability, downloads, or popups.
...
- `WaitEngine`: keeps page-state waits and now has URL, persisted navigation-event, document load-state, selector-state, console, dialog, network-quiet, request, and response primitives. The remaining work is broader navigation lifecycle semantics, layout stability, downloads, and popups.
...
6. Expand workflow waits to broader navigation lifecycle semantics, layout stability, dialogs, downloads, and popups. URL, persisted navigation-event, document load-state, selector-state, console, native JavaScript dialog, network-quiet, request, and response waits now exist.
...
- Waits: broader navigation lifecycle semantics, layout stability, downloads, popups, and deeper timeout/diagnostic e2e coverage for the current URL/navigation-event/load-state/selector/console/dialog/network/request/response MCP waits.
...
This is now a working real-browser automation foundation for top-document, open shadow-root, and same-origin iframe targets, including nested same-origin frame coverage, frame-ref discovery, stale frame-ref recovery, explicit unsupported-frame diagnostics, production-flow preflight, and first-class URL/navigation-event/load-state/selector/console/network-quiet/request/response waits. It is still not a finished Playwright/Cypress-equivalent engine. The remaining required work is full locator parity, broader navigation lifecycle waits beyond document readiness, deeper diagnostics, broader frame edge coverage, navigation-grade frame stability, and broader actionability parity.
